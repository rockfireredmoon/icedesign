...
public class ArmourColourBar extends Element {
	...
	public ArmourColourBar(BaseScreen mgr, Region region, AbstractCreature creature) {
		super(mgr);
	...
	void setDefinition(ClothingTemplate def) {
		...
		if (def != null) {
			...
			if (area != null) {
				final String tintPath = Icescene.checkAssetPath(String.format("%1$s/%2$s-%3$s-Tint.png.colormap",
						def.getKey().getPath(), def.getKey().getName(), area));
				try {
					final ColorMapConfiguration clothingMapColors = ColorMapConfiguration
							.get(getScreen().getApplication().getAssetManager(), tintPath);
					...
					for (final RGB rgb : mapColors) {
						ColorButton cfc = new ColorButton(screen, IceUI.toRGBA(rgb), false) {
							...
							protected void onChangeColor(ColorRGBA newColor) {
								...
								// Populate the list with the default colors if
								// there are none or not enough
